...
/* Read the shadow stack pointer register and assign its value to SSP.
   Return true if reading was successful, false otherwise.  */

static bool
amd64_linux_get_shadow_stack_pointer (gdbarch *gdbarch, CORE_ADDR *ssp)
{
...
{
  CORE_ADDR ssp;
  if (!amd64_linux_get_shadow_stack_pointer (gdbarch, &ssp))
    return;

  /* The shadow stack grows downwards.  To push addresses on the stack,
     we need to decrement SSP.
     For x32 the shadow stack elements are still 64-bit aligned.  Thus, we
     cannot use gdbarch_addr_bit to compute the new stack pointer.  */
  const bfd_arch_info *binfo = gdbarch_bfd_arch_info (gdbarch);
  const int bytes_per_word = (binfo->bits_per_word / binfo->bits_per_byte);
  const CORE_ADDR new_ssp = ssp - bytes_per_word;
  ...
  const int addr_size = gdbarch_addr_bit (gdbarch) / binfo->bits_per_byte;
...
static value *
amd64_linux_dwarf2_prev_ssp (frame_info_ptr this_frame, void **this_cache,
			     int regnum)
{
  ...
  value *retval = value::allocate (register_type (gdbarch, regnum));
  retval->set_lval (lval_register);
