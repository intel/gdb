...
breakpoints_should_be_inserted_now (void)
{
  if (gdbarch_has_global_breakpoints (current_inferior ()->arch ()))
    ...
    {
      if (may_leave_breakpoints_inserted (current_inferior ()->arch ()))
	{
...
bpstat::bpstat (const bpstat &other)
  : next (NULL),
    ...
    hit_lane_mask (other.hit_lane_mask)
{
...
bool
bpstat::find_hit_lane_mask (unsigned int &hit_lane_mask) const
{
	  ...
	  if (bs->is_cmd_for_all_lanes)
	    {
	      ...
	      for_simd_lanes (bs->hit_lane_mask, [&] (int lane)
		{
	    ...
	    {
	      int lane = find_first_active_simd_lane (bs->hit_lane_mask);
	      thread->set_current_simd_lane (lane);
...
code_breakpoint::code_breakpoint (struct gdbarch *gdbarch_,
				  ...
				  int thread_, int simd_lane_num_, int task_,
				  int inferior_, int ignore_count_,
				  int from_tty,
...
/* Given TOK, a string specification of condition and thread, as accepted
   ...
   If the breakpoint specification has an associated thread, SIMD lane number,
   task, or inferior, these are extracted into *THREAD, *SIMD_LANE_NUM, *TASK,
   and *INFERIOR respectively, otherwise these arguments are set to -1
   (for THREAD and INFERIOR) or 0 (for TASK).
...
find_condition_and_thread (const char *tok, CORE_ADDR pc,
			   gdb::unique_xmalloc_ptr<char> *cond_string,
			   int *thread, int *simd_lane_num, int *inferior,
			   int *task,  bool *inferior_pending,
      ...
      else if (toklen >= 1 && strncmp (tok, "inferior", toklen) == 0)
	{
	  ...
	  *inferior = strtol (tok, &tmptok, 0);
	  ...
	  if (pending_break_support == AUTO_BOOLEAN_FALSE
	      && valid_global_inferior_id (*inferior))
		   ...
		   unsigned flags)
{
  ...
  try
	...
	/* If the breakpoint must only be installed only for a specific
	   inferior, filter found sals first to only take into account those
	   with the matching pspace.  If list remains empty after that,
	   treat this as a pending breakpoint.  */
	...
	if (inferior > 0)
	  {
	    ...
	    auto new_end = remove_if (begin (canonical.lsals),
		end (canonical.lsals),
		[] (linespec_sals &lsals)
		  {
...
ordinary_breakpoint::print_it (const bpstat *bs) const
{
  ...
  if (show_thread_that_caused_stop () && bs->hit_lane_mask != 0x0)
    {
      if (inferior_thread ()->has_simd_lanes ()
...
breakpoint::print_recreate_thread (struct ui_file *fp) const
{
  ...
  if (inferior != -1)
    gdb_printf (fp, " inferior %d", inferior);
