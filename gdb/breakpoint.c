...
static void print_breakpoint_location (breakpoint *bp, bp_location *loc);
...
remove_breakpoints (void)
{
  ...
  if (debug_breakpoints)
    fprintf_unfiltered (gdb_stdlog, "breakpoints: remove_breakpoints ()\n");
...
remove_breakpoints_inf (inferior *inf)
{
  ...
  if (debug_breakpoints)
    fprintf_unfiltered (gdb_stdlog, "remove_breakpoints_inf (%d)\n", inf->num);
...
find_condition_and_thread_for_sals (const std::vector<symtab_and_line> &sals,
				    ...
				    int *task, int *inferior, char **rest)
{
  int num_failures = 0;
  for (auto &sal : sals)
    {
...
create_breakpoint (struct gdbarch *gdbarch,
		   ...
		   const char *cond_string,
		   int thread, int simd_lane, const char *extra_string,
...
break_command_1 (const char *arg, int flag, int from_tty)
{
  ...
  create_breakpoint (get_current_arch (),
		     location.get (),
		     nullptr, 0, -1, arg, false, 1 /* parse arg  */,
...
update_global_location_list (enum ugll_insert_mode insert_mode)
{
  ...
  if (debug_breakpoints)
    {
      fprintf_unfiltered (
	  gdb_stdlog, "breakpoints: update_global_location_list (%s)\n",
		  ...
		  struct program_space *search_pspace, int *found)
{
  ...
  if (exception.reason == 0 || exception.error != NOT_FOUND_ERROR)
      ...
      if (b->condition_not_parsed && b->extra_string != NULL)
	{
	  ...
	  int thread, task, simd_lane_num, inferior;
