	...
	case DW_OP_INTEL_regs:
	  {
	    ...
	    dwarf_require_integral (vregno->type ());
	...
	case DW_OP_INTEL_piece_stack:
	  {
	    ...
	    dwarf_require_integral (vsize->type ());
	    const ULONGEST size
	      = extract_unsigned_integer (vsize->contents (),
					  byte_order);
	...
	case DW_OP_INTEL_bit_piece_stack:
	  {
	    ...
	    dwarf_require_integral (voffset->type ());
	    const ULONGEST off
	      = extract_unsigned_integer (voffset->contents (),
					  byte_order);
	    /* Record the piece.  */
	    add_piece (size, off);
	...
	case DW_OP_INTEL_push_bit_piece_stack:
	  {
	    ...
	    const int addr_bit_size = this->m_addr_size * 8;
		...
		goto no_push;
	      ...
	      case DWARF_VALUE_LITERAL:
		{
		  const ULONGEST bitsize = this->m_len * 8;
		  if (bitsize < (size + off))
		  ...
		  result = 0;
		  copy_bitwise ((gdb_byte *) &result,
				(this->m_addr_size * 8) - size,
				this->m_data, off, size, byte_order);
		}
		break;
