		 ...
		 bfd_boolean big_p, unsigned int *addr_space)
{
  ...
  struct type *start_addr_type = check_typedef (v->type ());
  struct gdbarch *gdbarch = start_addr_type->arch ();

  /* For targets with multiple address spaces.  */
  if (gdbarch_address_space_from_type_flags_p (gdbarch))
    {
      if (start_addr_type->is_pointer_or_reference ())
	start_addr_type = start_addr_type->target_type ();

      addr_space_start_addr = gdbarch_address_space_from_type_flags (
					gdbarch,
					start_addr_type->instance_flags ());
    }
  ...
  while (*s != '\0')
      ...
      if (gdbarch_address_space_from_type_flags_p (gdbarch))
	{
	  ...
	  struct type *pattern_type = check_typedef (v->type ());

	  addr_space_t_pattern = gdbarch_address_space_from_type_flags (
					gdbarch,
					pattern_type->instance_flags ());
