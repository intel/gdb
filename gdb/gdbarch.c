	       ...
	       struct gdbarch_tdep *tdep)
{
  ...
  gdbarch->can_leave_breakpoints = false;
...
gdbarch_dump (struct gdbarch *gdbarch, struct ui_file *file)
{
  ...
  fprintf_filtered (file,
                      "gdbarch_dump: gdbarch_infcall_bp_address_p() = %d\n",
                      gdbarch_infcall_bp_address_p (gdbarch));
  fprintf_filtered (file,
                      "gdbarch_dump: infcall_bp_address = <%s>\n",
                      host_address_to_string (gdbarch->infcall_bp_address));
  fprintf_filtered (file,
...
gdbarch_can_step_over_breakpoint (struct gdbarch *gdbarch)
{
  ...
  /* Skip verify of can_step_over_breakpoint, invalid_p == 0 */
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_can_step_over_breakpoint called\n");
...
unsigned int
gdbarch_address_space_from_type_flags (struct gdbarch *gdbarch, type_instance_flags type_flags)
{
  gdb_assert (gdbarch != NULL);
...
gdbarch_core_load_hook (struct gdbarch *gdbarch, bfd *abfd)
{
  ...
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_core_load_hook called\n");
...
gdbarch_can_leave_breakpoints (struct gdbarch *gdbarch)
{
  ...
  /* Skip verify of can_leave_breakpoints, invalid_p == 0 */
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_can_leave_breakpoints called\n");
...
bool
gdbarch_infcall_bp_address_p (struct gdbarch *gdbarch)
{
  gdb_assert (gdbarch != NULL);
  return gdbarch->infcall_bp_address != NULL;
}

CORE_ADDR
gdbarch_infcall_bp_address (struct gdbarch *gdbarch, thread_info *tp)
{
  gdb_assert (gdbarch != NULL);
  gdb_assert (gdbarch->infcall_bp_address != NULL);
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_infcall_bp_address called\n");
  return gdbarch->infcall_bp_address (gdbarch, tp);
}

void
set_gdbarch_infcall_bp_address (struct gdbarch *gdbarch,
                                gdbarch_infcall_bp_address_ftype infcall_bp_address)
{
  gdbarch->infcall_bp_address = infcall_bp_address;
}
...
gdbarch_reserve_stack_space (struct gdbarch *gdbarch, const type *valtype, CORE_ADDR &sp)
{
  ...
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_reserve_stack_space called\n");
