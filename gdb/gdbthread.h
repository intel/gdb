...
public:
  ...
  lanes_mask_t active_simd_lanes_mask ();
...
extern int find_first_active_simd_lane (lanes_mask_t mask);

/* Return true, if LANE is unmasked in MASK.  */
extern bool is_simd_lane_active (lanes_mask_t mask, int lane);
...
extern void foreach_lane (lanes_mask_t mask, std::function<bool (int)> func,
			  simd_lane_order order = simd_lane_order::ASCENDING);
