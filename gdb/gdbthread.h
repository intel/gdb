...
public:
  ...
  bool is_simd_lane_active (int lane, frame_info_ptr frame = nullptr);

  /* Return true if current SIMD lane is active in this thread.  The selected
     frame is considered to determine if the lane is active.  */
  bool is_current_lane_active ();
...
public:
  ...
  explicit scoped_restore_current_simd_lane (thread_info *tp = nullptr);
  ~scoped_restore_current_simd_lane ();
  DISABLE_COPY_AND_ASSIGN (scoped_restore_current_simd_lane);
private:
  thread_info_ref m_tp;
  int m_simd_lane_num;
  bool m_was_active;
};
...
/* Execute function FUNC for all unmasked lanes in MASK.
   The integer argument of FUNC is the lane number, which corresponds
...
extern void for_simd_lanes (unsigned int mask, std::function<bool (int)> func,
