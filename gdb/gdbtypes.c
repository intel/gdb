...
resolve_dynamic_icc_pointer_or_reference (type *orig_type, type* resolved_type,
					  ...
					  int top_level)
{
  if (TYPE_DATA_LOCATION (TYPE_TARGET_TYPE (orig_type)) != nullptr)
    {
	...
	= resolve_dynamic_type_internal (TYPE_TARGET_TYPE (orig_type), pinfo,
					 top_level);
}
...
recursive_dump_type (struct type *type, int spaces)
{
  ...
  switch (TYPE_SPECIFIC_FIELD (type))
	...
	printf_filtered ("%*sfunc_type_flags %u\n", spaces, "",
