...
/* AMX tilecfg_raw register?  */

static bool
i386_tilecfg_raw_regnum_p (struct gdbarch *gdbarch, int regnum)
{
  i386_gdbarch_tdep *tdep = gdbarch_tdep<i386_gdbarch_tdep> (gdbarch);
  return regnum == tdep->tilecfg_raw_regnum;
}

/* AMX tiledata register?  */

static bool
i386_tiledata_regnum_p (struct gdbarch *gdbarch, int regnum)
{
  i386_gdbarch_tdep *tdep = gdbarch_tdep<i386_gdbarch_tdep> (gdbarch);
  return regnum == tdep->tiledata_regnum;
}
...
static struct type *
i386_tmm_type (struct gdbarch *gdbarch, int regnum)
{
  i386_gdbarch_tdep *tdep = gdbarch_tdep<i386_gdbarch_tdep> (gdbarch);
  uint8_t rows = tilecfg_reg::MAX_ROWS;
  uint16_t bytes_per_row = tilecfg_reg::MAX_BYTES_PER_ROW;
  const int tilenum = regnum - tdep->tmm_regnum;

  gdb_assert (tilenum >= 0 && tilenum < I387_NUM_TMM_REGS);

  /* Use tilecfg to determine the dimensions for tmm.  If the register cache
     hasn't been initialized, tilecfg can't be read yet.  */
  if (tdep->i386_tmm_type_init[tilenum])
    {
      gdb::byte_vector tilecfg_buf (register_size (gdbarch, tdep->tilecfg_raw_regnum), 0);
	...
	error (_("Could not read tilecfg to determine tmm dimensions."));
      ...
      tilecfg_reg tilecfg (tilecfg_buf.data ());

      /* We want to show the whole tile in case it isn't configured.  */
      if (tilecfg.rows ((uint8_t) tilenum) != 0)
	rows = tilecfg.rows ((uint8_t) tilenum);
      if (tilecfg.bytes_per_row ((uint8_t) tilenum) != 0)
	bytes_per_row = tilecfg.bytes_per_row ((uint8_t) tilenum);
    }
  ...
  union __gdb_builtin_type_matrix1024i
    {
      int8_t m_int8[rows][bytes_per_row];
  ...
  type *t = arch_composite_type (gdbarch, "builtin_type_tile", TYPE_CODE_UNION);
  ...
  append_composite_type_field
    (t, "m_uint8",
     init_vector_type (init_vector_type (bt->builtin_uint8, bytes_per_row),
		       rows));

  append_composite_type_field
    (t, "m_int32",
     init_vector_type
      (init_vector_type (bt->builtin_int32, bytes_per_row / 4), rows));
  ...
  append_composite_type_field
    (t, "m_fp32",
     init_vector_type
      (init_vector_type (bt->builtin_float, bytes_per_row / 4), rows));

  t->set_is_vector (true);
  t->set_is_dyn_register (true);
  t->set_name ("builtin_type_tile");
  tdep->i386_tmm_type_init[tilenum] = true;
...
i386_pseudo_register_type (struct gdbarch *gdbarch, int regnum)
{
  if (i386_mmx_regnum_p (gdbarch, regnum))
    ...
    return i386_tmm_type (gdbarch, regnum);
  else if (i386_tilecfg_regnum_p (gdbarch, regnum))
			  ...
			  const struct reggroup *group)
{
  ...
  int fp_regnum_p, mmx_regnum_p, xmm_regnum_p, mxcsr_regnum_p,
      ...
      sse_p, pkru_regnum_p, tilecfg_regnum_p, tmm_regnum_p,
      tilecfg_raw_regnum_p, tiledata_regnum_p;
  ...
  tiledata_regnum_p = i386_tiledata_regnum_p (gdbarch, regnum);
  tilecfg_raw_regnum_p = i386_tilecfg_raw_regnum_p (gdbarch, regnum);
		       ...
		       struct tdesc_arch_data *tdesc_data)
{
  ...
  const struct tdesc_feature *feature_sse, *feature_avx, *feature_avx512,
			     ...
			     *feature_amx, *feature_pl3_ssp, *feature_apx;
