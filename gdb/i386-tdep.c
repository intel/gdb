...
i386_tmm_type (struct gdbarch *gdbarch, int regnum)
{
  ...
  const uint16_t tilenum = regnum - tdep->tmm_regnum;
  ...
  if (tdep->i386_tmm_type_init[tilenum])
	...
	error (_("Could not read tilecfg to determine tmm dimensions."));
      ...
      if (tilecfg.rows (tilenum) != 0)
	rows = tilecfg.rows (tilenum);
      if (tilecfg.bytes_per_row (tilenum) != 0)
	bytes_per_row = tilecfg.bytes_per_row (tilenum);
    }
