...
inferior_pid_to_str (inferior *inf)
{
  ...
  if (pid != 0)
	  ...
	  && gdbarch_is_inferior_device (target_gdbarch))
	{
	  ...
	  const std::string pci_slot = device_info->pci_slot;

	  if (device_info->subdevice_id.has_value ())
	    return string_printf ("device [%s].%d", pci_slot.c_str (),
				  *(device_info->subdevice_id));
	  else
	...
	connection_id_len = conn.size ();
	 ...
	 later.  */
      device_in_inferiors = (device_in_inferiors
			     || gdbarch_is_inferior_device (inf->arch ()));
...
struct add_inferior_opts
{
  ...
  /* When true, the inferior stays hidden until it receives an event
     from its target.  */
  bool hidden = false;
};
...
connection."),
  ...
  gdb::option::flag_option_def<add_inferior_opts> {
    "hidden",
    [] (add_inferior_opts *opts) { return &opts->hidden; },
    N_("\
If specified, the inferior stays hidden until it receives an event\n\
from its target."),
  },
};
...
add_inferior_command (const char *args, int from_tty)
{
  ...
  for (unsigned int i = 0; i < opts.num_copies; ++i)
    {
      inferior *inf = add_inferior_with_spaces (opts.hidden);
