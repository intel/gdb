      ...
      while (1)
	{
	  ...
	  for (int i = 0; i < waits_needed; i++)
	      ...
	      else
		{
		  ...
		  /* We are calling 'mark_non_executing_threads' below, which
		     uses the current top target.  Also, 'setup_inferior' below
		     uses the current inferior.  So, do a context switch.  */
		  switch_to_thread_no_regs (t);
		  ...
		  /* This is needed to mark all the relevant threads in
		     case the event is received from an all-stop target.  */
	  ...
	  SWITCH_THRU_ALL_UIS ()
	      ...
	      if (current_thread->has_simd_lanes ())
		{
		  if (lanes_mask == 0x0)
		    ...
		    {
		      int lane = current_thread->current_simd_lane ();
