  ...
  if (value != new_value)
    {
      ...
      gdb::observers::command_param_changed.notify (param_name.c_str (),
						    c_str ());
      return true;
    }
...
set_schedlock_shortcut_option (const char *shortcut)
{
  ...
  if (any_changed)
    gdb::observers::command_param_changed.notify ("scheduler-locking",
						  shortcut);
}
      ...
      if (thread->has_pending_waitstatus ())
	{
	  infrun_debug_printf
	    ("thread %s has pending wait "
	     ...
	     thread->ptid.to_string ().c_str (),
	     thread->pending_waitstatus ().to_string ().c_str (),
