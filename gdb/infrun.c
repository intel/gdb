	...
	if (stop_soon == NO_STOP_QUIETLY)
	  {
	    ...
	    if (stop_on_solib_events)
	      {
		if (non_stop && ecs->ptid.is_pid ())
		  {
		    /* In non-stop mode, if the load event is generated by the
		       whole process, and no thread should be stopped.  In
		       this case, just print the solib changes, no stop is
		       required.  */
		    print_solib_change ();
