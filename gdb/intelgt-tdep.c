...
static void intelgt_check_reg_status (register_status status, int reg_num,
  ...
  for (struct so_list *so : current_program_space->solibs ())
	...
	continue;
      ...
      func (so->addr_low, so->addr_high-so->addr_low,
	    read, write, exec, modified, false, obfd);
    }
			  ...
			  const std::string &error_message)
{
  if (status == REG_UNAVAILABLE)
    throw_error (NOT_AVAILABLE_ERROR,
		 _("%s Register %d is not available."), error_message.c_str (),
		 reg_num);

  if (status == REG_UNKNOWN)
    error (_("%s Register %d is unknown."), error_message.c_str (), reg_num);
}
