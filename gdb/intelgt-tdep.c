  ...
  for (int argnum = 0; argnum < nargs; ++argnum)
	  ...
	  <= retval_regnum + INTELGT_MAX_GRF_REGS_FOR_ARGS)
	{
	  ...
	  if (is_a_promotable_small_struct (arg_type,
					    PROMOTABLE_STRUCT_MAX_SIZE_ARG))
	    {
	      /* The stucture will be stored in the GRF registers with SoA
		 layout.  Example:
	      ...
	      for (int field_idx = 0; field_idx < n_fields; ++field_idx)
		{
		  ...
		  int lane_offset = current_lane * field_len;

		  field_regnum += (reg_offset + lane_offset) / retval_regsize;
		  field_reg_offset = ((reg_offset + lane_offset)
				      % retval_regsize);

		  /* Prepare the TARGET_REGNUM and the REG_OFFSET for
		     the next field.  */
		  target_regnum += ((reg_offset + mem_occupation)
				    / retval_regsize);
		  reg_offset = (reg_offset + mem_occupation) % retval_regsize;
		  ...
		  regcache->cooked_write_part (
		    field_regnum, field_reg_offset, field_len,
		    (val + current_pos));
		}
	    }
	    ...
	    /* The argument has been pushed to the FE stack, and its
	      ...
	      int lane_offset = current_lane * address_size;
	      int reg_offset = lane_offset % retval_regsize;

	      /* Move to write on the right register.  */
	      target_regnum += lane_offset / retval_regsize;

	      /* Write to the corresponding part of register.  */
	      regcache->cooked_write_part (
		target_regnum, reg_offset, address_size,
		(gdb_byte *) &obj_addrs[obj_index++]);
	    }
	  else if (arg_type->is_vector ())
	    {
	      /* Vectors are copied to GRFs with the Structure of Arrays (SoA)
		 layout.  */

	      /* Length in bytes of an element in the vector.  */
	      int target_type_len = TYPE_LENGTH (TYPE_TARGET_TYPE (arg_type));
	      /* Number of elements in the vector.  */
	      int n_elements_to_write = len / target_type_len;

	      for (int element_idx = 0; element_idx < n_elements_to_write;
		   ++element_idx)
		{
		  ...
		  int reg_offset = total_offset % retval_regsize;

		  /* Move to write on the right register.  */
		  target_regnum = regnum + total_offset / retval_regsize;

		  /* Write to the corresponding part of register.  */
		  regcache->cooked_write_part (
		    target_regnum, reg_offset, target_type_len,
	  ...
	  else if (len <= 8)
	    {
	      ...
	      /* Move to write on the right register.  */
	      target_regnum = regnum + lane_offset / retval_regsize;

	      regcache->cooked_write_part (target_regnum, reg_offset, len,
					   val);
	    }
	  else
...
intelgt_gdbarch_init (gdbarch_info info, gdbarch_list *arches)
{
  ...
  set_gdbarch_can_leave_breakpoints (gdbarch, false);
