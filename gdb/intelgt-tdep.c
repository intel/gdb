			    ...
			    const char *error_message)
{
  ...
  gdb_byte *bytes = (gdb_byte *) alloca (size);
  gdb::array_view<gdb_byte> view (bytes, size);
  ...
  extract_integer (gdb::make_array_view (buffer, size), view,
		   gdbarch_byte_order (arch), false);
}
			     ...
			     const char *error_message)
{
  ...
  extract_integer (view, gdb::make_array_view (buffer, size),
		   gdbarch_byte_order (arch), false);
...
struct intelgt_gdbarch_data
{
  ...
  {
    constexpr int max_grf_count = 512;
    constexpr int min_grf_count = 16;
    int grf_count = regset_ranges[intelgt::regset_grf].end;
    if ((grf_count > max_grf_count) || (grf_count < min_grf_count))
  ...
  if (type_length * simd_width
	...
	return RETURN_VALUE_ABI_RETURNS_ADDRESS;
	 ...
	 type length).  */
      ...
      struct_elt elt
	= lookup_struct_elt (data->get_framedesc_type (regcache), "fe_sp", 0);
      CORE_ADDR addr = 0;
      intelgt_read_register_part (regcache, framedesc_regnum, elt.offset / 8,
				  elt.field->type ()->length (),
				  (gdb_byte *) &addr,
				  _("Cannot read fe_sp from framedesc."));

      CORE_ADDR reserved_struct_memory = align_up (type_length * simd_width,
						   OWORD_SIZE);
      if (addr < reserved_struct_memory)
	error (_("Invalid stack address of return value: %s"),
	       paddress (gdbarch, addr));
...
static const struct frame_unwind_legacy intelgt_unwinder =
  {
    "intelgt prologue",
    NORMAL_FRAME,			/* type */
    FRAME_UNWIND_ARCH,
...
intelgt_print_insn (bfd_vma memaddr, struct disassemble_info *info)
{
  ...
  iga_context_t iga_ctx = get_intelgt_gdbarch_data (gdbarch)->iga_ctx;
  if (iga_ctx == nullptr)
    {
      gdb_printf (_("\nDisassemble feature not available: libiga64 "
		    "context is invalid.\n"));
...
/* Build the NT_PRSTATUS for IntelGT.  */
			     ...
			     int *note_size)
{
  ...
  for (solib &so : current_program_space->solibs ())
	...
	{
	  FILE *so_file = fopen (so.name.c_str (), "r");
	  if (so_file == nullptr)
	    error ("Could not open the file %s", so.name.c_str ());
	  ...
	  fclose (so_file);
	  if (rsize != so_size)
	    error ("Failed to read %s", so.name.c_str ());
	}
...
intelgt_core_load_hook (struct gdbarch *gdbarch, bfd *abfd)
{
  ...
  iterate_file_mappings (
      &contents, long_type, abfd,
      [=] (int count)
       {
       },
      [=] (int i, const bfd_build_id *build_id, const file_mapping &fm)
       {
...
{
  gdb::array_view<field> fields = struct_type->fields ();
...
intelgt_dummy_id (struct gdbarch *gdbarch, const frame_info_ptr &this_frame)
{
  ...
  intelgt_gdbarch_data *data = get_intelgt_gdbarch_data (gdbarch);
  int framedesc_regnum = data->framedesc_base_regnum ();
  ...
  regcache *regcache = get_thread_regcache (inferior_thread ());
  struct_elt elt
    = lookup_struct_elt (data->get_framedesc_type (regcache), "fe_fp", 0);
  CORE_ADDR fe_fp = extract_unsigned_integer (buf + elt.offset / 8,
					      elt.field->type ()->length (),
					      byte_order);
...
intelgt_unwind_sp (gdbarch *gdbarch, const frame_info_ptr &next_frame)
{
  ...
  struct_elt elt
    = lookup_struct_elt (data->get_framedesc_type (regcache), "fe_sp", 0);
  CORE_ADDR fe_sp
    = extract_unsigned_integer (raw_bytes + (elt.offset / 8),
				elt.field->type ()->length (), byte_order);
  return fe_sp;
}
...
intelgt_run_ret_inst (gdbarch *gdbarch)
{
  ...
  if (target_write_memory (ret_inst_addr, buff.data (), buff.size ()) != 0)
    error (_("Target failed to inject a dummy RET instruction at %s"),
	   paddress (gdbarch, ret_inst_addr));
			 ...
			 void **dtor_data)
{
  ...
  lanes_mask_t mask = intelgt_active_lanes_mask (gdbarch, inferior_thread ());
  if (mask > UINT32_MAX)
    error (_("Active lanes mask 0x%" PRI_lanes_mask " is out of range."), mask);
  uint32_t current_ce = (uint32_t) mask;
  ...
  *dtor_data = infcall_cleanup_data;
  ...
  if (err != 0)
    error (_("Target failed to inject a dummy calla instruction at %s"),
	   paddress (gdbarch, calla_addr));
			 ...
			 CORE_ADDR struct_addr)
{
  ...
  struct_elt elt
    = lookup_struct_elt (data->get_framedesc_type (regcache), "fe_fp", 0);
  intelgt_write_register_part (regcache, framedesc_regnum, elt.offset / 8,
			       elt.field->type ()->length (),
			       (gdb_byte *) &fe_sp,
			       _("Cannot store FE frame pointer."));

  /* Update the FE stack pointer (framedesc.fe_sp).  */
  elt = lookup_struct_elt (data->get_framedesc_type (regcache), "fe_sp", 0);
  intelgt_write_register_part (regcache, framedesc_regnum, elt.offset / 8,
			       elt.field->type ()->length (),
			       (gdb_byte *) &fe_sp,
				  ...
				  gdb_byte *buff_write, type *valtype)
{
  ...
  gdb::array_view<field> fields = valtype->fields ();
  ...
  for (int field_idx = 0; field_idx < n_fields; ++field_idx)
	...
	intelgt_read_register_part (m_regcache, field_regnum,
				    field_reg_offset, field_len,
				    buff_write + current_pos,
				    _("Cannot read a register."));

      /* Write to the corresponding part of register.  */
      else if (buff_read != nullptr)
	intelgt_write_register_part (m_regcache, field_regnum, field_reg_offset,
				     field_len, buff_read + current_pos,
				     _("Cannot store a register"));
    }
}
  ...
  for (int element_idx = 0; element_idx < n_elements; ++element_idx)
    {
      ...
      /* Read from the corresponding part of register.  */
      if (buff_write != nullptr)
	intelgt_read_register_part (m_regcache, target_regnum, reg_offset,
				    element_len,
	...
	intelgt_write_register_part (m_regcache, target_regnum, reg_offset,
				     element_len,
...
intelgt_implicit_args_find_value_pair (gdbarch *gdbarch, thread_info *tp)
{
  ...
  std::unordered_map<std::string, implicit_args_value_pair>::iterator it
    = implicit_args_cache.find (key);
  ...
  if (it == implicit_args_cache.end ())
    {
      intelgt_implicit_args::read_args (gdbarch, tp);
      it = implicit_args_cache.find (key);
      gdb_assert (it != implicit_args_cache.end ());
    }
...
struct convert<zeinfo::kernel> {
  static bool decode (const Node &kernel_node, zeinfo::kernel &rhs)
  {
    ...
    /* execution_env has to have a scalar grf_count.  */
    if (!kernel_node["execution_env"]["grf_count"]
	|| !kernel_node["execution_env"]["grf_count"].IsScalar ())
      {
	dprintf ("%s grf_count for kernel \"%s\" is missing or invalid "
		 "but required.", error_prefix, rhs.name.c_str ());
	return false;
      }
    rhs.grf_count = kernel_node["execution_env"]["grf_count"].as<uint32_t> ();
...
static LONGEST
intelgt_core_xfer_siginfo (gdbarch *gdbarch, struct bfd &cbfd,
			   gdb_byte *readbuf, ULONGEST offset, ULONGEST len)
{
  ...
  asection *section
    = bfd_get_section_by_name (&cbfd, section_name.c_str ());
  if (section == nullptr)
    return -1;
				  ...
				  CORE_ADDR to, regcache *regs)
{
  ...
  switch (device_version)
	...
	switch (device_version)
	  ...
	  case intelgt::XE_INVALID:
	    /* We already have a device switch above, it should not be
	       possible to end up here.  */
	    gdb_assert_not_reached ("Unsupported device id 0x%" PRIx32,
				    device_id);
	  }
      }
      break;
...
intelgt_get_next_pcs (regcache *regcache)
{
  gdbarch *gdbarch = regcache->arch ();
  CORE_ADDR pc = regcache_read_pc (regcache);
...
/* Bit position for sign extension.  */
constexpr int INTELGT_PPGTT_MSB = 56;
/* Bits 63-57 set to 1 for sign extension.  */
constexpr CORE_ADDR INTELGT_CANONICAL_UPPER_BITS
  = (CORE_ADDR)0xFE00000000000000ULL;
/* Bits 56-0 mask (clears bits 63-57).  */
constexpr CORE_ADDR INTELGT_CANONICAL_LOWER_MASK
  = (CORE_ADDR)0x01FFFFFFFFFFFFFFULL;

/* Normalize Intelgt GPU addresses to canonical form.

   Canonical form requires sign-extending from bit 56: if bit 56 is set,
   bits 63-57 must all be 1; if clear, bits 63-57 must all be 0.

   Corefiles generated by corefilegen-gpu use canonical addresses in
   PT_LOAD segments, but register values may contain non-canonical forms.
   This function ensures address lookups work correctly by normalizing to
   canonical form.  */

static CORE_ADDR
intelgt_remove_non_address_bits_memory (gdbarch *gdbarch, CORE_ADDR addr)
{
  /* Only required for corefiles.  */
  if (!current_program_space->core_bfd ())
    return addr;

  /* Sign-extend from bit 56: if set, fill bits 63-57 with 1s; else clear
     them.  */
  return ((addr & ((CORE_ADDR)1 << INTELGT_PPGTT_MSB))
	  ? (addr | INTELGT_CANONICAL_UPPER_BITS)
	  : (addr & INTELGT_CANONICAL_LOWER_MASK));
}
...
Device vendor id and target id not found in intelgt target description."));
  ...
  /* Normalize Intelgt GPU addresses to canonical form.  This hook is
     essential for core file reads to work with addresses in non-canonical
     forms.  */
  set_gdbarch_remove_non_address_bits_memory
    (gdbarch, intelgt_remove_non_address_bits_memory);
