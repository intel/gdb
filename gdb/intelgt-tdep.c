...
public:
  ...
  CORE_ADDR alloc (size_t size) const
  {
	...
	if (!head->reserved && (head->size > size))
	  ...
	  {
	    /* No need to create a new block, just reuse this one.  */
	    head->reserved = true;
	    break;
	  }
...
intelgt_active_lanes_mask (struct gdbarch *gdbarch, thread_info *tp)
{
  ...
  uint32_t sr0_2 = 0;
  gdb::array_view<gdb_byte> view ((gdb_byte *) &sr0_2, sizeof (sr0_2));
  intelgt_read_register_part (thread_regcache, data->sr0_regnum,
			      sizeof (uint32_t) * 2, view,
			      _("Cannot read dispatch mask"));
			       ...
			       value **read_value, const gdb_byte *writebuf)
{
  ...
  intelgt_gdbarch_tdep *tdep = gdbarch_tdep<intelgt_gdbarch_tdep> (gdbarch);
  int retval_regnum = tdep->retval_regnum;
  ...
  if (type_length * simd_width
	...
	return RETURN_VALUE_ABI_RETURNS_ADDRESS;
	 ...
	 type length).  */
      const int framedesc_regnum = tdep->framedesc_base_regnum ();
      int32_t addr = 0;
      intelgt_read_register_part (regcache, framedesc_regnum, 24,
				  gdb::make_array_view ((gdb_byte *) &addr,
							sizeof (addr)),
	...
	error ("Invalid stack address of return value: 0x%x", addr);
...
intelgt_dummy_id (struct gdbarch *gdbarch, const frame_info_ptr &this_frame)
{
  ...
  intelgt_gdbarch_tdep *tdep = gdbarch_tdep<intelgt_gdbarch_tdep> (gdbarch);
  int framedesc_regnum = tdep->framedesc_base_regnum ();
...
intelgt_run_ret_inst (gdbarch *gdbarch)
{
  ...
  type *void_type
    = type_allocator (gdbarch).new_type (TYPE_CODE_VOID,
					 ...
					 "dummy_ret");
  type *func_void_type = lookup_function_type (void_type);
...
intelgt_infcall_dummy_dtor (void *data, int unused)
{
  ...
  try
      ...
      if (stopped_by_random_signal)
	{
	  ...
	  gdb::array_view<const gdb_byte> ip_view ((gdb_byte *) &return_ip,
						   sizeof (return_ip));
	  intelgt_write_register_part (regcache, framedesc_regnum, 0,
				       ip_view,
				       _("Cannot write return_ip."));

	  /* Update the RETURN_MASK to reflect the caller CE.  */
	  gdb::array_view<const gdb_byte> mask_view ((gdb_byte *) &return_mask,
						     sizeof (return_mask));
  ...
  if (value_type->name () && strcmp (value_type->name (), "dummy_ret") == 0)
    {
      ...
      gdb::array_view<gdb_byte> ip_view ((gdb_byte *) &return_ip,
					 sizeof (return_ip));
      intelgt_read_register_part (regcache, framedesc_regnum, 0, ip_view,
				  _("Cannot read return ip"));
      *real_pc = funaddr;
			 ...
			 CORE_ADDR struct_addr)
{
  ...
  intelgt_gdbarch_tdep *tdep = gdbarch_tdep<intelgt_gdbarch_tdep> (gdbarch);
  const int framedesc_regnum = tdep->framedesc_base_regnum ();
  const int retval_regnum = tdep->retval_regnum;
  ...
  intelgt_write_register_part (regcache, framedesc_regnum, 16,
			       gdb::make_array_view ((gdb_byte *) &fe_sp, 8),
			       _("Cannot update FE frame pointer"));

  /* Update the FE stack pointer (framedesc.fe_sp).  */
  intelgt_write_register_part (regcache, framedesc_regnum, 24,
			       gdb::make_array_view ((gdb_byte *) &fe_sp, 8),
			       _("Cannot update FE stack pointer"));
  return fe_sp;
}
  ...
  for (int field_idx = 0; field_idx < n_fields; ++field_idx)
	...
	intelgt_read_register_part (m_regcache, field_regnum,
				    ...
				    (buff_write + current_pos,
				     field_len),
				    _("Cannot read GRF register"));

      /* Write to the corresponding part of register.  */
      else if (buff_read != nullptr)
	intelgt_write_register_part (m_regcache, field_regnum,
				     field_reg_offset,
				     gdb::make_array_view
				     (buff_read + current_pos,
				      field_len),
				     _("Cannot write GRF register"));
    }
}
			     ...
			     gdb_byte *buff_write, int len)
{
  ...
  if (buff_write != nullptr)
    intelgt_read_register_part (m_regcache, target_regnum, reg_offset,
				gdb::make_array_view (buff_write, len),
				_("Cannot read GRF register"));

  /* Write to the corresponding part of the register.  */
  else if (buff_read != nullptr)
    intelgt_write_register_part (m_regcache, target_regnum, reg_offset,
				 gdb::make_array_view (buff_read, len),
				 _("Cannot write GRF register"));
}
