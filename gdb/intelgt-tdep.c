...
{
  regcache *regcache = get_thread_arch_regcache (tp->inf->process_target (),
						 tp->ptid, gdbarch);
  CORE_ADDR pc = regcache_read_pc (regcache);
  obj_section *section = find_pc_section (pc);
  if (section == nullptr)
  ...
  const gdb::optional<std::string> kernel_name
...
intelgt_software_single_step (regcache *regcache)
{
  ...
  /* FIXME: Favor hardware single-stepping over software stepping.
