		      ...
		      gdb_byte *readbuf, const gdb_byte *writebuf)
{
  ...
  if (readbuf != nullptr)
      ...
      if (valtype->is_vector ())
	{
	  ...
	  int target_type_length = valtype->target_type ()->length ();
...
intelgt_unwind_pc (gdbarch *gdbarch, const frame_info_ptr &next_frame)
{
  ...
  if (regcache->cooked_read (data->isabase_regnum,
			     &isabase) != REG_VALID)
      ...
      else
	{
	  ...
	  if (is_a_promotable_small_struct (arg_type,
	  ...
	  else if (arg_type->is_vector ())
	    {
	      ...
	      int target_type_len = arg_type->target_type ()->length ();
	      /* Number of elements in the vector.  */
