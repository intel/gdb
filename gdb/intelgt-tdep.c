...
struct intelgt_gdbarch_data
{
  ...
  /* Returns regnum where frame descriptors are stored.  */
  int
  framedesc_base_regnum ()
  {
    ...
    bfd *abfd = obj->obfd;
    int machine = get_elf_backend_data (abfd)->elf_machine_code;
...
/* Filter VMAs to be dumped to a core file.  Currently only memory backing
   loaded shared libraries is written - this is likely to be extended later.  */
			     ...
			     find_memory_region_ftype func, void *obfd)
{
  for (struct so_list *so : current_program_space->solibs ())
    {
      if ((so->so_name == nullptr) || (so->so_name[0] == '\0'))
	continue;
      int read = 1;
      int write = 0;
      int exec = 1;
      int modified = 1;
      func (so->addr_low, so->addr_high-so->addr_low,
	    read, write, exec, modified, obfd);
    }
			     ...
			     CORE_ADDR &sp)
{
  ...
  sp = align_up (sp + TYPE_LENGTH (value_type) * simd_width, OWORD_SIZE);
			...
			const unsigned int simd_width)
{
  ...
  /* Length in bytes of an element in the vector.  */
  int target_type_len = TYPE_LENGTH (TYPE_TARGET_TYPE (valtype));
  /* Number of elements in the vector.  */
      ...
      if (buff_read != nullptr)
	{
	  ...
	  if (err != 0)
	    error ("Target failed to write struct on the stack: "
		   "type %s of length %lu",
		   valtype->name (), TYPE_LENGTH (valtype));
	}
      else if (buff_write != nullptr)
	{
	  /* Write the current field on the stack.  */
