...
static value *
intelgt_dwarf2_prev_ce (frame_info *this_frame, void **this_cache,
			int regnum)
{
  gdbarch *gdbarch = get_frame_arch (this_frame);
...
intelgt_core_load_hook (struct gdbarch *gdbarch, bfd *abfd)
{
  ...
  iterate_file_mappings (
	 ...
	 gdb_bfd_ref_ptr mem_bfd (gdb_bfd_open_from_target_memory (
	       fm.vaddr, fm.size, "elf64-intelgt", fm.filename));
	 printf_filtered (_("Loading object file embedded into "
	 ...
	 objfile *objf = symbol_file_add_from_bfd (mem_bfd.get (), fm.filename,
						   0, &sai,
						   OBJF_SHARED, NULL);
