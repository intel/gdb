...
jit_unregister_code (struct gdbarch *gdbarch, CORE_ADDR entry_addr)
{
  ...
  if (jit_debug)
    gdb_printf (gdb_stdlog, "jit_unregister_code (%s)\n",
		host_address_to_string (jited));
  if (jited == nullptr)
    gdb_printf (gdb_stderr,
		_ ("Unable to find JITed code entry at address: %s\n"),
		paddress (gdbarch, entry_addr));
  else
    {
      gdb::observers::jit_object_unloaded.notify (current_program_space,
						  jited);
      jited->unlink ();
    }
}
...
maint_jit_dump_command (const char *args, int from_tty)
{
  ...
  struct objfile *obj = block->objfile ();
