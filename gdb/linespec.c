	...
	if (filter[i])
	  {
		...
		- the provided line is above the declaration line of the
		  function
	       ...
	       The intent of this heuristic is that a breakpoint requested
  ...
  for (const auto &elt : ls->file_symtabs)
    {
      ...
      symtab_index_vector imv = find_line_symtabs (elt, val.line);
      symtab *matching_symtab = (!imv.empty () ? imv[0].first : nullptr);
