   ...
   read_core_file_mappings_loop_ftype  loop_cb)
{
  ...
  struct type *ulong_type
    = init_integer_type (alloc, gdbarch_long_bit (gdbarch), 1, "ulong");
  ...
  iterate_file_mappings (&contents, ulong_type, cbfd, pre_loop_cb,
			 [=] (int i, const bfd_build_id *build_id,
			      const file_mapping &fm)
			 {
			   ...
			   loop_cb (i, fm.vaddr, fm.vaddr + fm.size,
				    fm.offset, fm.filename, build_id);
			 });
}
			      ...
			      const std::string &filename, void *data)
{
  ...
  map->add ({vaddr, size, offset, filename.c_str ()});
