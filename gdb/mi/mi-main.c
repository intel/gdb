...
void
mi_cmd_thread_simd_width (const char *command, const char *const *argv,
			  int argc)
{
  if (argc != 0)
    error (_("-thread-simd-width: No arguments required."));
  ...
  if (tp->has_simd_lanes ())
    {
      ...
      uiout->field_unsigned ("simd-width", width);
    }
			      ...
			      int argc)
{
  ...
  if (bp != nullptr
    ...
    uiout->field_fmt ("hit-lanes-mask", "0x%" PRI_lanes_mask, hit_lane_mask);
  else if (tp->executing ())
    error (_("Thread %d is not stopped."), tp->global_num);
  else
...
mi_cmd_thread_info (const char *command, const char *const *argv, int argc)
{
  ...
  info_threads_opts it_opts {false, false, enable_stopped, enable_qid,
			     enable_lid};
...
mi_cmd_execute (struct mi_parse *parse)
{
  ...
  std::optional<scoped_restore_tmpl<bool>> restore_suppress_notification
