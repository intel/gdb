...
static symtab *
lookup_symtab_prioritize_non_empty_linetable (program_space *pspace,
					      const char *name)
{
  symtab *result = nullptr;
  iterate_over_symtabs (pspace, name, [&] (symtab *s)
    {
      if (result == nullptr)
	result = s;
			  ...
			  int argc)
{
  ...
  s = lookup_symtab_prioritize_non_empty_linetable (current_program_space,
						    filename);
