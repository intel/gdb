...
class IntelgtAutoAttach(gdb.Breakpoint):
    ...
    def get_env_variable(var, default=None):
        ...
        env_var = gdb.execute("show env %s" % var, to_string=True)
        if env_var.find("Environment variable \"%s\" not defined."
                        % var) != -1:
            return default
        return env_var.replace("%s = " % var, "", 1).strip()
...
INTELGT_AUTO_ATTACH_DISABLE=1 can be used for disabling auto-attach.""")
        ...
        gdb.execute("remove-inferior %d" % gt_inf.num)
    ...
    def handle_attach_gdbserver_gt(self, connection, inf):
        ...
        gdbserver_gt_attach_str = "%s --multi --hostpid=%d - " \
                                  % (self.gdbserver_gt_binary, inf.pid)

        if gdbserver_gt_path_env_var:
            gdbserver_gt_attach_str = \
              "%s/%s" % (gdbserver_gt_path_env_var, gdbserver_gt_attach_str)
        ...
        if igfxdbg_lib_path_env_var:
            ld_lib_path_str = "LD_LIBRARY_PATH=%s:$LD_LIBRARY_PATH" \
            % igfxdbg_lib_path_env_var
    ...
    @staticmethod
    def make_native_gdbserver(inf, ld_lib_path, gdbserver_cmd):
        """Spawn and connect to a native instance of gdbserver-gt."""
        # Switch to the gt inferior.  It is the most recent inferior.
        gt_inf = gdb.inferiors()[-1]
        gdb.execute("inferior %d" % gt_inf.num, False, True)
        gdb.execute("set sysroot /")
        target_remote_str = " ".join(("%s %s" % (ld_lib_path,
                                                 gdbserver_cmd)).split())
        target_cmd = "target extended-remote"

        gdb.execute("%s | %s" % (target_cmd, target_remote_str), False, True)
        print("intelgt: gdbserver-gt started for process %d." % inf.pid)
    ...
    def make_remote_gdbserver(self, inf, ld_lib_path, gdbserver_cmd):
        ...
        if not remote_target_env_var:
            ...
            if connection_target is None:
                ...
                # Connection over tcp.
                target_str = "%s %s%s" % (remote_protocol_str,
                                          remote_username_str,
                                          connection_target)
            ...
            target_str = "%s %s%s" % (remote_protocol_str,
                                      remote_username_str,
                                      remote_target_env_var)
            gdb.execute("set remotetimeout 20")

        target_remote_str = " ".join(("%s %s %s" % (target_str, ld_lib_path,
                                                    gdbserver_cmd)).split())

        gdb.execute("target extended-remote | %s" % target_remote_str)
        print("""\
intelgt: inferior %d created remotely for process %d using command
'%s'""" % (gt_inf.num, inf.pid, target_remote_str))
    ...
    def init_gt_inferiors(self):
        ...
        if connection not in ('remote', 'native'):
            print("intelgt: connection name '%s' not recognized." % connection)
...
intelgt: the igfxdcd module (i.e. the debug driver) is not loaded.""")
        ...
        if host_inf not in self.inf_dict or self.inf_dict[host_inf] is None:
            ...
            try:
                ...
                gdb.execute("inferior %d" % (host_inf.num), False, True)
                if not self.is_nonstop:
    ...
    def set_suppress_notifications(new_value):
        ...
        info = gdb.execute("maint show suppress-notification-cli", False, True)
        was_cli_suppressed = info.endswith("on.\n")
        gdb.execute("maint set suppress-notification-cli " + new_value)
