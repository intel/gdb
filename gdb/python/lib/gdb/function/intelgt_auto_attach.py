...
\033[0m""")
    ...
    def execute_with_errorcapture(command, capture_output=True):
        ...
        try:
            ...
            gdbserver_cmd = command.split("|")
            if (len(gdbserver_cmd) > 1
                and "gdbserver-ze" in gdbserver_cmd[1]):
                # PIPE is used for stdin to avoid issue due to user input
                # during the wait for termination.
                with subprocess.Popen(gdbserver_cmd[1],
                                      shell=True,
                                      stdin=subprocess.PIPE) as gdbserver_proc:
                    # The gdbserver-ze will terminate due to CalledProcessError
                    # or TimeoutExpired exception will occur in case problem is
                    # in the gdb-oneapi.
                    try:
                        gdbserver_proc.wait(5)
                    except (RuntimeError, subprocess.CalledProcessError,
                            subprocess.TimeoutExpired):
                        if gdbserver_proc is not None:
                            kill(gdbserver_proc.pid, signal.SIGINT)

            print(f"\033[93mFailed to execute the command: '{command}'\033[0m")
...
class IntelgtAutoAttach:
    ...
    def __init__(self):
        ...
        gdb.events.post_attach.connect(self.handle_post_attach_event)
        self.inf_dict = {}
        ...
        self.live_attach_started = False
    ...
    def set_gdbserver_ze_path(self):
        """Set path variables to the installed gdbserver-ze locations and
        later gdbserver-ze installation takes precendence in the same order
        just in case multiple binaries are available for the gdbserver-ze:
        1) Check 'INTELGT_AUTO_ATTACH_GDBSERVER_PATH' in the inferior's
        environment.
        2) Check gdbserver-ze in the debugger installation path."""
...
\033[0m""")
            self.gdbserver_path_env_var = \
                    self.get_env_variable("INTELGT_AUTO_ATTACH_GDBSERVER_GT_PATH")
        self.gdbserver_path_env_var = \
          self.get_env_variable("INTELGT_AUTO_ATTACH_GDBSERVER_PATH")

        auto_attach_path = path.dirname(__file__)
        gdbserver_path = path.join(
            auto_attach_path, "..", "..", "..", "..", "..", "..",
                "debugger", "bin")
        if path.exists(path.join(gdbserver_path, "gdbserver-ze")):
            self.gdbserver_install_path = gdbserver_path
    ...
    def handle_new_objfile_event(self, event):
        ...
        if ('libze_loader.so' in event.new_objfile.filename or
            ...
            DebugLogger.log(
                f"received {event.new_objfile.filename} loaded event. "
                "Setting up bp hook.")
            hook = self.setup_hook_bp(f"-qualified zeModuleCreate inferior "
    ...
    @DebugLogger.log_call
    def handle_post_attach_event(self, event):
        """Handler for GDB's 'post attach' event.  If the host was
        attached, we init gt inferiors here and make sure it is done
        only once at a time.  Because the make_native_gdbserver function
        during initialization also sends the attach command which can
        be ignored."""
        if event is None:
            return

        if self.gt_inferior_init_pending and not self.live_attach_started:
            self.live_attach_started = True
    ...
    def handle_attach_gdbserver_gt(self, inf):
        ...
        if self.gdbserver_path_env_var:
            gdbserver_attach_str = \
              f"{self.gdbserver_path_env_var}/{gdbserver_attach_str}"
        elif self.gdbserver_install_path:
            gdbserver_attach_str = \
    ...
    def make_native_gdbserver(self, inf, gdbserver_cmd):
        ...
        if gdbserver_port:
            ...
            if (IntelgtErrorReport.execute_with_errorcapture(
                    f"target extended-remote | {target_remote_str}",
                    capture_output)):
