...
public:
  ...
  int remote_multi_address_space_p (remote_state*) const;
  ...
  if (packet_idx >= 0 && packet_idx  < PACKET_MAX)
    {
	 ...
	 show_packet_config_cmd
	   (file, &remote->m_features.m_protocol_packets[packet_idx]);
	 else
	   show_packet_config_cmd
	     (file, &remote_protocol_packets[packet_idx]);
	return;
    }
...
int
remote_features::remote_multi_address_space_p (remote_state *rs) const
{
  ...
  for (i = 0; i < PACKET_MAX; i++)
    {
      ...
      remote_protocol_packets[i].name = nullptr;
      remote_protocol_packets[i].title = nullptr;
      remote_protocol_packets[i].show_cmd = nullptr;
      remote_protocol_packets[i].set_cmd = nullptr;
    }
}
			    ...
			    bool detach_fork)
{
  ...
  if ((fork_kind == TARGET_WAITKIND_FORKED && m_features.remote_fork_event_p (rs))
			 ...
			 struct notif_event *event)
{
  ...
  if (!remote->m_features.remote_multi_process_p (rs)
				       ...
				       unsigned int addr_space = 0)
{
  ...
  if (addr_space != 0)
    {
      if (m_features.remote_multi_address_space_p (rs))
	{
	  /* Append "@".  */
	  *p++ = '@';
  ...
  *p++ = 'm';
  ...
  if (addr_space != 0)
  {
    if (m_features.remote_multi_address_space_p (rs))
      {
	*p++ = '@';
	p += hexnumstr (p, (ULONGEST) addr_space);
      }
    else
      {
	/* If the remote doesn't support access requests to different memory
	   spaces we need to error out.  We can't just read from the default
	   space, as the value would be wrong.  */
	error (_("Remote server does not support reading from non-default \n"
		 "address spaces."));
      }
  }
			      ...
			      CORE_ADDR *found_addrp, unsigned int addr_space)
{
  ...
  auto read_memory = [=] (CORE_ADDR addr, gdb_byte *result, size_t len,
    ...
    {
      return (target_read (this, TARGET_OBJECT_MEMORY, NULL, result, addr,
			   len, l_addr_space) == len);
    };
  ...
  /* Insert header.  */
  if (m_features.remote_multi_address_space_p (rs) && addr_space != 0)
...
remote_target::ack_library (const char *name)
{
  ...
  switch (packet_ok (rs->buf, &remote_protocol_packets[PACKET_vAck_library]))
    {
    case PACKET_OK:
      break;
    case PACKET_UNKNOWN:
...
remote_target::ack_in_memory_library (CORE_ADDR begin, CORE_ADDR end)
{
  ...
  struct packet_config *packet
    = &remote_protocol_packets[PACKET_vAck_in_memory_library];
...
Show the maximum size of the address (in bits) in a memory packet."), NULL,
  ...
  add_packet_config_cmd (&remote_protocol_packets[PACKET_R], "R", "restart",
			 0);
  ...
  add_packet_config_cmd (&remote_protocol_packets[PACKET_vAck_library],
			 "vAck:library", "ack-library", 0);

  add_packet_config_cmd (&remote_protocol_packets[PACKET_vAck_in_memory_library],
