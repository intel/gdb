...
remote_target::remote_query_delta_thread_list ()
{
  ...
  packet_result result
    = m_features.packet_ok (rs->buf, PACKET_qDeltaThreadList);
  switch (result.status ())
    {
    case PACKET_OK:
      if (strcmp (rs->buf.data (), "1") == 0)
...
remote_target::skip_frame (remote_state *rs)
{
  ...
  while (1)
	...
	case 'E':		/* Looks like this is an error packet.  */
	  ...
	  if (packet_check_result (buf, false).status () == PACKET_ERROR)
...
test_packet_check_result ()
{
  ...
  SELF_CHECK (packet_check_result ("E000", true).status () == PACKET_ERROR);
