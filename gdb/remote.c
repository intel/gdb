...
remote_target::add_current_inferior_and_thread (const char *wait_status)
{
  ...
  remote_add_inferior (fake_pid_p, curr_ptid.pid ());
...
remote_target::decode_expedites (stop_reply *stop_reply, thread_info *thr)
{
  ...
  struct regcache *regcache = get_thread_arch_regcache (this, thr->ptid,
							gdbarch);
				   ...
				   struct target_waitstatus *status)
{
  *status = stop_reply->ws;
  ...
  if (status->kind () != TARGET_WAITKIND_EXITED
	...
	{
	  if (thr != nullptr)
	    decode_expedites (stop_reply, thr);
	  stop_reply->expedites_cache.clear ();
	}
...
remote_target::fetch_registers_using_e (regcache *regcache)
{
  ...
  switch (m_features.packet_ok (rs->buf, PACKET_e))
    {
    case PACKET_OK:
      break;
    case PACKET_UNKNOWN:
			      ...
			      CORE_ADDR *found_addrp, unsigned int addr_space)
{
  ...
  /* Insert header.  */
  if (m_features.remote_multi_address_space_p () && addr_space != 0)
...
remote_target::btrace_sync_conf (const btrace_config *conf)
{
  ...
  if ((m_features.packet_support (PACKET_Qbtrace_conf_pt_event_tracing)
	...
	= packets_descriptions[PACKET_Qbtrace_conf_pt_event_tracing].name;
      ...
      if (m_features.packet_ok (buf, PACKET_Qbtrace_conf_pt_event_tracing)
	  == PACKET_ERROR)
	{
	  if (buf[0] == 'E' && buf[1] == '.')
...
remote_target::ack_library (const char *name)
{
  ...
  switch (m_features.packet_ok (rs->buf, PACKET_vAck_library))
    {
    case PACKET_OK:
      break;
    case PACKET_UNKNOWN:
...
remote_target::ack_in_memory_library (CORE_ADDR begin, CORE_ADDR end)
{
  ...
  switch (m_features.packet_ok (rs->buf, PACKET_vAck_in_memory_library))
    {
    case PACKET_OK:
      break;
    case PACKET_UNKNOWN:
...
Show the maximum size of the address (in bits) in a memory packet."), NULL,
  ...
  add_packet_config_cmd (PACKET_Qbtrace_conf_pt_ptwrite,
