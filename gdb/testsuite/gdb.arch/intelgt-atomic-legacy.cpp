...
const int
get_index (int sg_startx, int sg_starty)
{
  return (sg_startx * m_tile) * N + sg_starty / sg_size * n_tile;
}
...
matrix_multiply (float *C, bfloat16 *A, bfloat16 *B, sycl::queue &deviceQueue)
{
  ...
  deviceQueue.submit ([&](sycl::handler &cgh)
	...
	[[sycl::reqd_sub_group_size (sg_size)]]
	{
	  ...
	  /* To generate an atomic sequence on some platforms the resulting
	     matrix has to be independent from the previous iteration.
	     Otherwise, sync instructions are added after each iteration.  */
	  for (int k = 0; k < n_k_tile; k++)
	    {
	      joint_matrix_store (
		sg, sub_c[k],
