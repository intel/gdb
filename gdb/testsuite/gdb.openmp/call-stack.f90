...
function first (num1, num2) result(total)
  ...
  integer  :: total
  total = second (num1 + 5, num2 * 3) ! ordinary-middle-loc
...
function inlined_first (num1, num2) result(total)
  ...
  integer, intent(in) :: num1, num2
  integer :: total
  total = inlined_second (num1 + 5, num2 * 3) ! inlined-middle-loc
  total = total + 30
...
program call_stack
  ...
  !$omp& map(tofrom: data) private(ten, four, fourteen)
    ten = data(1) + 2;
    four = data(2) - 5;
    fourteen = ten + four;
    data(0) = first (fourteen + 1, 3); ! ordinary-outer-loc
