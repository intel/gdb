...
{
public:
  explicit scoped_inc_dec_ref (const std::vector<tp_emask> &tp_emask_vec)
    : m_tp_emask_vec (tp_emask_vec)
  {
    for (tp_emask saved : m_tp_emask_vec)
      saved.tp->incref ();
  }
...
private:
  const std::vector<tp_emask> &m_tp_emask_vec;
};
...
static bool
tp_array_compar_ascending (const tp_emask &a_tp_emask, const tp_emask &b_tp_emask)
{

  const thread_info *a = a_tp_emask.tp;
  const thread_info *b = b_tp_emask.tp;
...
static bool
tp_array_compar_descending (const tp_emask &a_tp_emask, const tp_emask &b_tp_emask)
{
  const thread_info *a = a_tp_emask.tp;
  const thread_info *b = b_tp_emask.tp;
		   ...
		   const qcs_flags &flags)
{
  ...
  std::string thr_header =
    string_printf (_("\nThread %s (%s"), print_thread_id (thr),
		   thread_target_id_str (thr).c_str ());

  try
    {
      thread_execute_cmd (thr, cmd, from_tty, flags);
    }
  catch (const gdb_exception_error &ex)
    {
      if (!flags.silent)
			    ...
			    simd_lane_kind lane_kind)
{
  ...
  if (tc != 0)
	...
	tp_emask_list_cpy.push_back ({tp, tp->active_simd_lanes_mask ()});

      gdb_assert (tp_emask_list_cpy.size () == tc);

      /* Increment the refcounts, and restore them back on scope
	 exit.  */
      scoped_inc_dec_ref inc_dec_ref (tp_emask_list_cpy);
...
static struct value *
simd_lane_num_make_value (struct gdbarch *gdbarch, struct internalvar *var,
			  void *ignore)
{
...
/* Implementation of `lane' variable.  */

static const struct internalvar_funcs simd_lane_funcs =
{
...
Show printing of thread events (such as thread start and exit)."), NULL,
  ...
  create_internalvar_type_lazy ("_simd_lane", &simd_lane_funcs, NULL);
}
