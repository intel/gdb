...
thread_info::set_default_simd_lane ()
{
  if (has_simd_lanes ())
    {
      int lane = (simd_lane_num >= 0) ? simd_lane_num : 0;
      lanes_mask_t active_mask = active_simd_lanes_mask ();
...
{
  lanes_mask_t mask = active_simd_lanes_mask ();
...
void
foreach_lane (lanes_mask_t mask, std::function<bool (int)> func,
...
int
find_first_active_simd_lane (lanes_mask_t mask)
{
  int result = -1;

  foreach_lane (mask, [&] (int lane)
    {
      result = lane;
...
bool
is_simd_lane_active (lanes_mask_t mask, int lane)
{
...
/* Print one row in info thread table.
   ...
   OPTS is options passed to the "info threads" command.
   CURRENT shows whether we print the current lane of the current thread.  */

static void
print_thread_row (ui_out *uiout, const info_threads_opts &opts,
		  thread_info *tp, thread_info *current_thread)
{
  int core;
      ...
      else
	{
	  ...
	  if (uiout->is_mi_like_p ())
	      ...
	      if (tp->has_simd_lanes ())
		{
		  ...
		  uiout->field_fmt ("execution-mask", "0x%" PRI_lanes_mask,
				    mask);
		 ...
		 thread_info *current_thread)
{
  ...
  /* Print single row.  */
  print_thread_row (uiout, opts, tp, current_thread);
}
...
{
  print_thread_info_1 (uiout, requested_threads, opts, global_ids, pid);
}
				   ...
				   thread_filter_parameters *filter_params)
{
  ...
  simd_lane_order lane_order
    = ascending ? simd_lane_order::ASCENDING : simd_lane_order::DESCENDING;
	  ...
	  if (for_all_lanes)
	    {
		 ...
		 skipped.  */
	      foreach_lane (saved.emask, [&] (int lane)
		{
		  switch_to_thread (tp);
			     ...
			     int global_ids)
{
  ...
  while (!parser.finished ())
	...
	is_simd_from_star = parser.in_simd_lane_star_range ();
      ...
      if (!parser.get_tid (&inf_num, &thr_num, &simd_lane_num))
	error (_("Failed to parse %s."),
	       std::string (tidlist, cmd - tidlist - 1).c_str ());
...
print_filtered_thread_ids (thread_filter_parameters *filter_params)
{
  ...
  using const_thread_iter
    = std::vector<std::pair<thread_info*, lanes_mask_t>>::const_iterator;
	  ...
	  if (tp->state == THREAD_STOPPED)
	    {
	      if (tp->is_unavailable ())
		uiout->text (") unavailable]");
	      else if (tp->has_simd_lanes () && !is_active)
		uiout->text (") inactive]");
	      else
		uiout->text (")]");
	    }
