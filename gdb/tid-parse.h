...
public:
  ...
  /* Parse a thread ID or a thread range list.  Optionally, parse
     ...
     3rd call: *INF_NUM=3; *THR_NUM=4 *SIMD_LANE_NUM=4 (finished==0)
     4th call: *INF_NUM=3; *THR_NUM=5 *SIMD_LANE_NUM=3 (finished==0)
     5th call: *INF_NUM=3; *THR_NUM=5 *SIMD_LANE_NUM=4 (finished==0)
...
private:
  ...
  /* The upper non-inclusive boundary for the SIMD lane parser.
     We expect up to 32 lanes.  */
