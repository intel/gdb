...
linux_process_target::post_set_resume_request (thread_info *thread)
{
  ...
  if (lwp->stopped
      ...
      && dequeue_one_deferred_signal (lwp, &lwp->status_pending))
    {
      ...
      threads_debug_printf ("Dequeueing deferred signal %d for LWP %ld, "
      ...
      if (lwp != NULL)
	{
	  ...
	  if (WIFSTOPPED (lwp->status_pending)
	      ...
	      if (thread->last_resume_kind != resume_step)
		{
		  ...
		  resume_one_nti (lwp, lwp->stepping, 0, nullptr);
		}
	      else
		threads_debug_printf
