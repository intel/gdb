	...
	for_each_thread (pid, [this, rkind, &nstopped] (thread_info *tp)
	  {
		...
		switch (rkind)
		  ...
		  default:
		    gdb_assert_not_reached ("unexpected resume kind");
		  }
  ...
  switch (state)
	...
	case resume_step:
	  ...
	  if (device->nresumed < device->nthreads)
	    {
	      device->nresumed++;
	      dprintf ("device %lu's nresumed=%ld%s", device->ordinal,
		       device->nresumed,
		       ((device->nresumed == device->nthreads)
			? " (all)" : ""));
	    }
    ...
    case ze_thread_state_unavailable:
      dprintf ("resume request for unavailable thread %d.%ld (%s)",
	       tp->id.pid (), tp->id.lwp (),
      ...
      else
	{
	  ...
	  if (!ze_has_priority_waitstatus (tp))
	      ...
	      if (rinfo.kind == resume_step)
		{
		  ...
		  if (start != end)
		    dprintf ("resuming thread %s for stepping in range "
			     "[0x%" PRIx64 ", 0x%" PRIx64 ")",
			     ze_thread_id_str (zetp->id).c_str (),
			     start, end);
		}
