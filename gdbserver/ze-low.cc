...
ze_thread_device (const thread_info *thread)
{
  ...
  process_info *process = thread->process ();
...
/* Normalize the resume request for easier processing later on.  */

static void
normalize_resume_info (thread_resume &rinfo)
{
  ptid_t rptid = rinfo.thread;
  ...
  for (size_t i = 0; i < n; ++i)
    {
      ...
      normalize_resume_info (rinfo);

      /* Log the normalized resume request.  */
